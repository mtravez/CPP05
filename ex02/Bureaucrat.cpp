...
void Bureaucrat::executeForm(const AForm &form)
{
	...
	{
		form.execute(*this);
		std::cout << LIME << getName() << RESET << " executed " << RED << form.getName() << RESET << std::endl;
	}
	catch (std::exception &e)
	{
		std::cout << LIME << getName() << RESET << " couldn't execute " << RED << form.getName() << RESET << " because " << e.what() << std::endl;
	}
}
